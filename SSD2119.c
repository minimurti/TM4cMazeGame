...
// ************** LCD_DrawTriangle **********************///////////////////////////////////////////////////MEMEME I Jordan Added this
// - Draws a Triangle centered at x0, y0
// ********************************************************
void LCD_DrawTriangle(unsigned short x, unsigned short y, short radius, unsigned short color){
    LCD_DrawLine(x-radius, y-radius, x, y+radius, color);//draw three lines around a center point
    LCD_DrawLine(x-radius, y-radius, x+radius, y-radius, color);
    LCD_DrawLine(x+radius, y - radius, x, y + radius, color);
}

void LCD_DrawRightTriangle(unsigned short x, unsigned short y, short radius, unsigned short color){
    LCD_DrawLine(x-radius, y+radius, x+radius, y, color);
    LCD_DrawLine(x-radius, y+radius, x-radius, y-radius, color);
    LCD_DrawLine(x-radius, y - radius, x + radius, y, color);
}
...
void LCD_DrawFilledTriangle(unsigned short x, unsigned short y, short radius, int direction,unsigned short color){
  ...
  switch (direction){
    case DOWN:
      for (radii = -radius; radii<=0 ; radii++){///repeately draw triangles of a shrinking radiu
        LCD_DrawTriangle(x, y, radii, color);
      }
      break;
   case UP:
      for (radii = 0; radii<=radius ; radii++){// -radius makes it flip
        LCD_DrawTriangle(x, y, radii, color);
      }
      break;
    case LEFT:
      for (radii = -radius; radii<=0 ; radii++){
        LCD_DrawRightTriangle(x, y, radii, color);
      }
      break;
   case RIGHT:
      for (radii = 0; radii<=radius ; radii++){
        LCD_DrawRightTriangle(x, y, radii, color);
      }
      break;
